...
import type { RankingStrategy, RankingsResponse } from '../types'

export function ResultsPanel({ version }: { version: number }) {
  const [strategy, setStrategy] = useState<RankingStrategy>('weighted_minmax')
  ...
  const load = useCallback(async () => {
    ...
    setIncomplete(null)
    try {
      const res = await api.analytics.rankings(strategy)
      setData(res)
    } catch (e) {
    ...
    <div className="panel">
      ...
      <div className="card form-row strategy-bar">
        <label>
          ...
          <select
            value={strategy}
            onChange={(e) => setStrategy(e.target.value as RankingStrategy)}
          >
      ...
      {data && !loading && (
        <>
          {data.message && !data.rankings?.length && (
            <div className="banner warn">{data.message}</div>
          )}
