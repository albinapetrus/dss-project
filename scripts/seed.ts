/**
 * Заповнює БД демонстраційним кейсом: ≥3 альтернативи, ≥3 критерії, повна матриця оцінок.
 * Запуск: MONGODB_URI=mongodb://127.0.0.1:27017/dss npx ts-node scripts/seed.ts
 */
...
const AlternativeSchema = new mongoose.Schema(
  ...
  { timestamps: true, collection: 'alternatives' },
);
...
const EvaluationSchema = new mongoose.Schema(
  {
    alternativeId: { type: mongoose.Schema.Types.ObjectId, ref: 'Alternative', required: true },
...
async function main() {
  ...
  const Alternative = mongoose.model('SeedAlternative', AlternativeSchema);
  const Criterion = mongoose.model('SeedCriterion', CriterionSchema);
  const Evaluation = mongoose.model('SeedEvaluation', EvaluationSchema);
  ...
  const byName = (items: { _id: mongoose.Types.ObjectId; name: string }[], n: string) =>
    items.find((x) => x.name === n)!._id;
